...
**产出**：
- ✅ 模板目录 `scripts/template-catalog.mjs`：双主题各 16（游戏角色：奇幻/赛博/仙侠/像素/二次元/机甲/废土/哥特/北欧/水墨/魔法学院/蒸汽朋克/星际/Q版/怪物图鉴/克苏鲁；盲盒：奶油/马卡龙/毛绒/果冻/国潮/星空/美食/圣诞/樱花/海洋/铁皮/宇航员/森林/水晶/机甲萌宠/烘焙）
- ✅ 批量生成脚本 `scripts/generate-template-assets.mjs`：每模板 1 参考图（1024px）+ 1 不同主体示范产出（640px，演示"换主体同风格"）；幂等（已存在跳过，不重复计费）；自动上传桶 + upsert 行 + 重写 seed.sql 模板段（markers 包裹）
- ✅ 64 张图零失败入库（仓库 ~10MB，db reset 自动入桶）；占位 SVG 删除
- ✅ 真实验收：模板库 32 卡全真图；`game-xianxia` 复刻「骑白鹤的小道童」风格继承精准

**满足验收**：01-mvp-scope 验收 #9「首批 30–50 条模板已上线，覆盖双主题」（32 ∈ [30,50]）。

**运营提示**：新增/调整模板改 `template-catalog.mjs` 后跑脚本即可（仅生成缺失素材）；`--force` 全量重生成；`--only slug` 定点重做。
